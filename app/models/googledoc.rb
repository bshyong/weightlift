...
class GoogleDoc
    ...
    field :file_obj, type: String
    field :worksheet_obj, type: String
    field :auth_tokens, type: String


    attr_accessor :session, :filename, :worksheet_name, :file_obj, :worksheet_obj, :auth_tokens

    def initialize(username, password, filename, worksheet_name)
        @session = GoogleDrive.login(username, password)
        @auth_tokens = @session.auth_tokens #store as string
        @filename = filename
        @worksheet_name = worksheet_name
        @file_obj = Marshal.dump(@session.spreadsheet_by_title(filename))
